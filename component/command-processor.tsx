...
const AGENT_TEXT_SHADOWS = {
   Agent Vision Æternum: '0 2px 4px rgba(255, 107, 107, 0.5)',
  Agent Solis Lux: '0 2px 4px rgba(72, 52, 212, 0.5)',
  Agent Necro Nexus: '0 2px 4px rgba(108, 92, 231, 0.5)',
   Agent Logos Infinitum: '0 2px 4px rgba(249, 168, 38, 0.5)'
}
...
export function AgentChat({ agent, role, onClose }: AgentChatProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (quantumAgent) {
      ...
      // Check if response is an error (e.g., invalid command)
      if (response.toLowerCase().includes('error')) {
        setCommandError('Invalid command or agent not recognized. Please try again.')
      } else {
